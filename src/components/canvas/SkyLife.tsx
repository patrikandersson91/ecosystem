...
function createCloudTexture() {
  ...
  const center = size / 2
  const grad = ctx.createRadialGradient(center, center, size * 0.12, center, center, size * 0.48)
...
function createBirdFlight(limit: number): BirdFlight {
  ...
  const endZ = MathUtils.clamp(startZ + MathUtils.randFloat(-limit * 0.16, limit * 0.16), -limit * 0.7, limit * 0.7)
...
export default function SkyLife() {
  ...
  const cloudSeeds = useMemo(() => Array.from({ length: CLOUD_COUNT }, () => randomCloud(SKY_LIMIT)), [])
  ...
  useFrame((_, rawDelta) => {
    ...
    for (let i = 0; i < cloudSeeds.length; i++) {
      ...
      const outsideMap = Math.abs(seed.x) > SKY_LIMIT || Math.abs(seed.z) > SKY_LIMIT
      if (outsideMap) {
      ...
      cloud.scale.set(seed.scale * seed.width, seed.scale * seed.height, seed.scale * seed.depth)
      cloud.rotation.y = Math.sin(seed.phase * 0.5) * 0.12
      cloud.rotation.z = Math.sin(seed.phase * 0.35) * 0.04
      cloud.visible = Math.abs(seed.x) <= SKY_LIMIT && Math.abs(seed.z) <= SKY_LIMIT
    }
    ...
    if (!flight.active) {
      ...
      if (flockRef.current) {
        ...
        if (flockRef.current.visible) {
          ...
          flockRef.current.rotation.z = Math.sin((t + flight.wingPhase) * Math.PI * 2.2) * 0.04
        }
      }

      for (let i = 0; i < flockBirds.length; i++) {
        const bird = flockBirds[i]
        const flap = Math.sin((t * 20 + flight.wingPhase + bird.flapOffset) * Math.PI * 2) * 0.66
      ...
      {cloudSeeds.map((seed, i) => (
        ...
        >
          {CLOUD_PUFFS.map((puff, puffIdx) => {
            ...
            return (
              <sprite key={puffIdx} position={[puff.x, puff.y, puff.z]} scale={[puffScale, puffScale, 1]}>
                <spriteMaterial
                  map={cloudTexture}
                  color="#f7fbff"
                  transparent
